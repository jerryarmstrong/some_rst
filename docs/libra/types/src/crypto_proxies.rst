...
//! solana_libra_crypto::traits::Signature. The container is an opaque NewType that
...
//!   in terms of solana_libra_crypto::traits, rather than accessing the implementation details of a particular
...
use solana_libra_crypto::{hash::HashValue, traits::Signature as RawSignature};
...
impl<Sig: RawSignature> SignatureWrapper<Sig> {
    ...
    pub fn try_from(
        bytes: &[u8],
    ) -> Result<Self, solana_libra_crypto::traits::CryptoMaterialError> {
